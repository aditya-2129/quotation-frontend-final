...
import { toolingRateService } from '@/services/rates';
import ActionButtons from '@/components/shared/ActionButtons';

export default function ToolingRatesPage() {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const res = await toolingRateService.listRates(limit, (page - 1) * limit);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Remove this tool from the master library?")) {
      try {
        await toolingRateService.deleteRate(id);
        fetchData();
      } catch (error) {
      ...
      <div className="flex flex-col gap-6">
        <div className="flex items-center justify-between p-4 bg-white border border-zinc-200 rounded-xl shadow-sm">
           <div className="relative w-full max-w-md">
              <input 
                 type="text" 
                 placeholder="Search tooling inventory..." 
           ...
           <div className="overflow-x-auto">
              <table className="w-full text-left text-sm border-collapse">
                 <thead className="bg-zinc-50 border-b border-zinc-200">
                    <tr>
                       <th className="px-6 py-4 text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Tool Descriptor / Item</th>
                       <th className="px-6 py-4 text-[10px] font-bold text-zinc-400 uppercase tracking-widest text-center">Unit Type</th>
                 ...
                 <tbody className="divide-y divide-zinc-200">
                    {isLoading ? [1,2,3,4,5].map(i => (
                       ...
                       <tr><td colSpan="4" className="px-6 py-20 text-center text-zinc-400 italic font-medium tracking-widest text-[11px] uppercase">No tooling master data records found.</td></tr>
                    ) : filteredRates.map(rate => (
                       <tr key={rate.$id} className="group hover:bg-zinc-50/80 transition-colors">
                          <td className="px-6 py-5">
                             <div className="flex flex-col">
                                <span className="text-zinc-950 font-bold">{rate.item_name}</span>
                             </div>
                          </td>
                          ...
                          <td className="px-6 py-5 text-right">
                             <span className="text-sm font-mono font-bold text-emerald-700 italic">₹{parseFloat(rate.rate).toFixed(2)}</span>
                          </td>
                          <td className="px-6 py-5 text-right">
      ...
      <div className="w-full max-w-md bg-white rounded-xl border border-zinc-200 shadow-2xl overflow-hidden">
        <header className="px-6 py-4 border-b border-zinc-100 bg-zinc-50/50">
           <h2 className="text-lg font-bold text-zinc-950 tracking-tight">
              {data ? 'Inventory Sync' : 'Register New Tooling Entity'}
           </h2>
        </header>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
           <div>
              <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1.5">Tool Descriptor Name *</label>
           ...
           <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                 <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1.5">Billing Unit</label>
           ...
           <div className="flex gap-3 pt-4 border-t border-zinc-100">
              ...
              <button type="submit" disabled={isSubmitting} className="flex-[2] h-10 rounded-lg bg-zinc-950 text-white font-bold shadow-lg disabled:opacity-50 transition-all uppercase italic text-xs">
                 {isSubmitting ? 'Syncing...' : 'Commit Inventory'}
              </button>
           </div>
        </form>
      </div>
