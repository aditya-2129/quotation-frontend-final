...
const DimensionInput = ({ label, field, value, allowance, onChange }) => (
  <div className="flex-1 min-w-[145px]">
     <label className="text-[7.5px] font-black text-zinc-400 uppercase block mb-1 flex justify-between items-center">
     ...
     <div className="flex gap-1">
        ...
        <input 
          ...
          step="0.01"
          placeholder="+ mm"
          className="w-[40%] h-8 bg-brand-primary/5 border border-brand-primary/20 border-l-0 rounded-r-lg px-2 text-[10px] font-black text-brand-primary outline-none focus:ring-1 focus:ring-brand-primary transition-all font-mono placeholder:text-brand-primary/40"
          value={allowance ?? ""}
...
const MaterialConfigurationRow = ({ item, idx, libraries, onUpdate }) => {
  ...
  // Server-side Material Search (handles large datasets)
  useEffect(() => {
        ...
        <div className="flex-1 p-2 flex flex-wrap items-center gap-4">
            ...
            <div className="w-48 border-r border-zinc-100 pr-4 flex items-center gap-3">
               {item.part_image ? (
                  ...
                  <div className="h-10 w-10 rounded-lg border-2 border-dashed border-zinc-100 bg-zinc-50 flex items-center justify-center flex-shrink-0">
                     <svg className="h-4 w-4 text-zinc-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                  </div>
               )}
               <div className="min-w-0">
                  <span className="text-[9px] font-black text-zinc-300 uppercase tracking-widest block mb-0.5">Part Name</span>
                  <h4 className="text-[13px] font-black text-zinc-950 truncate leading-tight mb-0.5">{item.part_name}</h4>
                  <span className="text-[8px] font-bold text-zinc-400 font-mono italic">REF: {item.id}</span>
               </div>
            </div>
            ...
            <div className={`flex-[1.5] min-w-[280px] transition-all duration-500 ${step > 1 && !isOpen ? 'opacity-80 scale-[0.99]' : ''}`}>
               <div className={`relative group ${isOpen ? 'z-[100]' : 'z-0'}`}>
                 <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest block mb-1.5 flex items-center gap-1">01. Pick Material Grade <span className="text-red-500 font-extrabold">*</span></span>
                 <div className="flex gap-2">
                    <div className="relative flex-1">
                       <input 
                        type="text"
                        className={`w-full h-9 pl-4 pr-10 rounded-xl bg-zinc-50 border focus:ring-1 focus:ring-brand-primary focus:bg-white outline-none transition-all font-bold text-black text-[11px] placeholder:font-normal ${isOpen ? 'border-brand-primary ring-1 ring-brand-primary bg-white' : 'border-zinc-200'}`}
                       ...
                       <div className="w-24 animate-in slide-in-from-left-2 duration-300">
                          <div className="relative">
                             ...
                             <input 
                               type="number"
                               step="0.01"
                               className="w-full h-9 pl-6 pr-2 rounded-xl bg-brand-primary/5 border border-brand-primary/20 focus:ring-1 focus:ring-brand-primary focus:bg-white outline-none transition-all font-black text-[12px] font-mono text-brand-primary placeholder:text-brand-primary/40"
                               placeholder="Rate"
                               value={(item.material.base_rate ?? "").toString()}
                               onChange={(e) => onUpdate({ material: { ...item.material, base_rate: parseFloat(e.target.value) || 0 } })}
                             />
                             <span className="absolute -top-3.5 right-0 text-[7px] font-black text-brand-primary uppercase tracking-tighter">Buying Price / kg</span>
                          </div>
                       </div>
                    )}
                 </div>
                     ...
                     <div className="absolute left-0 right-0 top-full mt-2 z-[70] bg-white border border-zinc-200 rounded-xl shadow-2xl max-h-64 overflow-y-auto animate-in slide-in-from-top-2 duration-300 ring-1 ring-black/5">
                        ...
                        {displayMaterials.length === 0 ? (
                           ...
                           >
                                  ...
                                  <span className="text-[9px] text-zinc-400 font-normal opacity-70 italic tracking-tight truncate">{m.name}</span>
                               </div>

                               <div className="w-1/3 flex justify-center">
                                  {(m.shape || m.form) && (
                                     <span className="text-[8px] px-2 py-0.5 rounded bg-zinc-100 text-zinc-600 font-black uppercase tracking-wider border border-zinc-200 shadow-sm-inset">
                                        {m.shape || m.form}
                                     </span>
                                  )}
                               </div>
                               ...
                               <div className="w-1/3 flex items-center justify-end gap-3">
                                  <span className="text-brand-primary font-black italic whitespace-nowrap">₹{m.base_rate} <span className="text-[8px] font-normal text-zinc-400 not-italic">/ kg</span></span>
           ...
           {(item.material || step >= 2) && (
              <div className={`transition-all duration-500 animate-in fade-in slide-in-from-left-4 ${step > 2 ? 'opacity-80 scale-[0.99]' : ''}`}>
                 <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest block mb-1.5 flex items-center gap-1">02. Select Shape <span className="text-red-500 font-extrabold">*</span></span>
           ...
           {item.shape && (
              <div className="flex-1 flex gap-4 items-center animate-in fade-in slide-in-from-left-4">
                 ...
                 <div className="text-right border-l border-zinc-100 pl-4 pr-2 py-1 flex flex-col justify-center min-w-[120px]">
                    <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest block mb-0.5">Sub-Total Cost</span>
                    <div className="text-[16px] font-black text-zinc-950 leading-tight">
                    ...
                    <div className="flex flex-col gap-0">
                       <span className="text-[9px] font-bold text-zinc-500 font-mono">{(item.material_weight || 0).toFixed(2)}kg @ ₹{(item.material?.base_rate || 0).toFixed(2)}</span>
                    </div>
                 </div>
              </div>
           )}
    ...
    <section className={`bg-white rounded-xl border transition-all duration-300 ${activePhase === 'material' ? 'border-zinc-300 shadow-xl ring-1 ring-zinc-200/50' : 'border-zinc-200 shadow-sm'}`}>
       ...
       >
          <div className="flex items-center gap-3">
             <span className={`flex h-6 w-6 items-center justify-center rounded-full text-[10px] font-black border transition-all duration-300 ${activePhase === 'material' ? 'bg-brand-primary border-brand-primary text-white shadow-lg shadow-brand-primary/20' : 'bg-white border-zinc-200 text-zinc-400'}`}>{panelIndex}</span>
             <h3 className={`text-[12px] font-black uppercase tracking-[0.2em] transition-colors ${activePhase === 'material' ? 'text-brand-primary' : 'text-zinc-500 group-hover:text-brand-primary'}`}>Material & Weight Selection</h3>
          </div>
          <div className="flex items-center gap-4">
             <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md border italic animate-in fade-in zoom-in duration-300 ${
             ...
             <svg className={`h-4 w-4 text-zinc-400 transition-transform duration-300 ${activePhase === 'material' ? 'rotate-180 text-brand-primary' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" /></svg>
          </div>
       </header>
