...
const OrderDetailsModal = ({ isOpen, onClose, order }) => {
  ...
  const getPoFileUrl = () => {
    if (!order.po_scan_file_id) return null;
  ...
  const InfoCard = ({ icon: Icon, label, value, subValue, color = "zinc" }) => (
    <div className={`flex items-center gap-3 p-4 rounded-2xl bg-${color}-50/50 border border-${color}-100`}>
      <div className={`h-10 w-10 rounded-xl bg-${color}-100 flex items-center justify-center text-${color}-600`}>
      ...
      <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-[32px] shadow-2xl overflow-hidden flex flex-col border border-zinc-200">
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <InfoCard icon={Hash} label="PO Reference" value={order.po_number} subValue={`QTN: ${order.quotation_no}`} color="brand-primary" />
